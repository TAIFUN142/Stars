...
BUY_IMAGE_URL = "https://i.pinimg.com/736x/65/09/47/65094754459cfa954d89a68ce89c8b15.jpg"
SELL_IMAGE_URL = "https://media.istockphoto.com/id/2108172267/ru/%D1%84%D0%BE%D1%82%D0%BE/%D1%82%D0%B5%D1%85%D0%BD%D0%BE%D0%BB%D0%BE%D0%B3%D0%B8%D1%8F-%D1%84%D0%BE%D0%BD%D0%BE%D0%B2%D0%BE%D0%B5-%D0%B8%D0%B7%D0%BE%D0%B1%D1%80%D0%B0%D0%B6%D0%B5%D0%BD%D0%B8%D0%B5-%D0%BA%D0%BE%D0%BD%D1%86%D0%B5%D0%BF%D1%86%D0%B8%D1%8F-%D0%B3%D1%80%D0%B0%D1%84%D0%B8%D0%BA-%D0%B1%D0%B8%D0%B7%D0%BD%D0%B5%D1%81-%D1%84%D0%B8%D0%BD%D0%B0%D0%BD%D1%81%D1%8B-%D0%B0%D0%BD%D0%B0%D0%BB%D0%B8%D0%B7-%D0%BC%D0%B0%D1%80%D0%BA%D0%B5%D1%82%D0%B8%D0%BD%D0%B3-%D0%B8-%D0%BF%D1%80%D0%B8%D0%B1%D1%8B%D0%BB%D1%8C.jpg?s=612x612&w=0&k=20&c=kx3NaYFWHCJAOBxW2sF6KHxBLpfwQLrA8fdIn1aeib0="
# ===============================
...
class MarketAnalyzer:
    ...
    def _detect_asset_type(self, asset):
        if "/OTC" in asset or asset.endswith(" OTC"):
            return "otc"
    ...
    def analyze_market(self, asset=None, timeframe=None):
        ...
        if len(df) < 60:
            return {
                ...
                "is_otc_proxy": ("/OTC" in asset or asset.endswith(" OTC"))
            }
...
def create_assets_menu():
    ...
    btn_crypto = types.InlineKeyboardButton("💰 Криптовалюты", callback_data="category_crypto")
    btn_forex = types.InlineKeyboardButton("💱 Форекс", callback_data="category_forex")
    btn_commodities = types.InlineKeyboardButton("🛢️ Сырье", callback_data="category_commodities")
    btn_indices = types.InlineKeyboardButton("📊 Индексы", callback_data="category_indices")
    btn_otc = types.InlineKeyboardButton("📊 OTC активы", callback_data="category_otc")
...
def _create_paginated_menu(items, prefix, page=1):
    markup = types.InlineKeyboardMarkup(row_width=2)
    items_per_page = 20
    ...
    for i in range(start_idx, end_idx, 2):
        if i + 1 < end_idx:
            btn1 = types.InlineKeyboardButton(items[i], callback_data=f"asset_{items[i]}")
            btn2 = types.InlineKeyboardButton(items[i + 1], callback_data=f"asset_{items[i + 1]}")
            markup.add(btn1, btn2)
        else:
            btn = types.InlineKeyboardButton(items[i], callback_data=f"asset_{items[i]}")
            markup.add(btn)

    navigation_buttons = []

    if page > 1:
        navigation_buttons.append(types.InlineKeyboardButton("◀️ Назад", callback_data=f"{prefix}_page_{page - 1}"))
    if end_idx < len(items):
        navigation_buttons.append(types.InlineKeyboardButton("Вперед ▶️", callback_data=f"{prefix}_page_{page + 1}"))
...
def create_crypto_menu(page=1):
    return _create_paginated_menu(CRYPTO_ASSETS, "crypto", page)


def create_forex_menu(page=1):
    return _create_paginated_menu(FOREX_ASSETS, "forex", page)


def create_commodities_menu(page=1):
    return _create_paginated_menu(COMMODITIES_ASSETS, "commodities", page)


def create_indices_menu(page=1):
    return _create_paginated_menu(INDICES_ASSETS, "indices", page)


def create_otc_menu(page=1):
    return _create_paginated_menu(OTC_ASSETS, "otc", page)
...
def start_command(message):
    try:
        ...
        welcome_text = f"""
🎯 ДОБРО ПОЖАЛОВАТЬ, {esc_md(user.first_name)}!
...
def process_pocket_id(message):
    try:
        ...
        admin_message = (
            ...
            f"ID: {user.id}\n"
            f"Имя: {user_dict.get('first_name', user.first_name)}\n"
        ...
        try:
            ...
            print("Не удалось отправить заявку админу:", e)
            bot.send_message(
                user.id,
...
👤 **ПРОФИЛЬ:**
├ Имя: {esc_md(user.first_name)}
├ ID: `{user.id}`
├ Статус: {status_text}
├ Pocket ID: {esc_md(pocket_id if pocket_id else 'не указан')}
├ Выбранное время: {esc_md(preferred_timeframe if preferred_timeframe else 'случайное')}
└ Регистрация: {esc_md(join_date)}
        ...
        if action == "approve":
            ...
            try:
                bot.edit_message_text(
                    ...
                    text=f"✅ **Верификация подтверждена**\n\nПользователь {esc_md(first_name)} (ID: {user_id}) успешно верифицирован.",
                    parse_mode="Markdown"
                )
            except Exception:
                pass
            ...
            try:
                bot.send_message(
                    user_id,
                    f"❌ **ВАША ВЕРИФИКАЦИЯ ОТКЛОНЕНА**\n\nУважаемый {esc_md(first_name)}, проверьте регистрацию и Pocket ID.",
                    parse_mode="Markdown"
                )
            except Exception:
                pass
            ...
            try:
                bot.edit_message_text(
                    chat_id=call.message.chat.id,
                    message_id=call.message.message_id,
                    text=f"❌ **Верификация отклонена**\n\nПользователь {esc_md(first_name)} (ID: {user_id}) не прошел верификацию.",
                    parse_mode="Markdown"
                )
            except Exception:
                pass
    ...
    try:
        ...
        if call.data == "category_crypto":
            ...
            bot.edit_message_text(
                ...
                parse_mode="Markdown",
                reply_markup=create_forex_menu(1)
            )
            ...
            bot.edit_message_text(
                ...
                parse_mode="Markdown",
                reply_markup=create_commodities_menu(1)
            )
            ...
            bot.edit_message_text(
                ...
                parse_mode="Markdown",
                reply_markup=create_indices_menu(1)
            )
...
def handle_asset_callback(call):
    try:
        ...
        if signal_data.get("unavailable"):
            bot.send_message(
                call.message.chat.id,
                f"⚠️ По активу **{esc_md(signal_data['asset'])}** сейчас нельзя построить сигнал.\n\n"
                f"Причина: {esc_md(signal_data['price_action'])}\n\n"
                f"Попробуйте другой актив или другой таймфрейм.",
                parse_mode="Markdown",
                reply_markup=create_main_menu()
            )
...
@bot.callback_query_handler(func=lambda call: call.data.startswith("forex_page_"))
def handle_forex_pagination(call):
    try:
        page = int(call.data.split("_")[2])
        bot.edit_message_text(
            chat_id=call.message.chat.id,
            ...
            text="💱 **Выберите валютную пару:**\n\nНажмите на актив для получения сигнала",
            parse_mode="Markdown",
            reply_markup=create_forex_menu(page)
        )
    except Exception:
        traceback.print_exc()
        bot.answer_callback_query(call.id, "❌ Произошла ошибка!")
...
@bot.callback_query_handler(func=lambda call: call.data.startswith("commodities_page_"))
def handle_commodities_pagination(call):
    try:
        page = int(call.data.split("_")[2])
        bot.edit_message_text(
            chat_id=call.message.chat.id,
            ...
            text="🛢️ **Выберите сырьевой актив:**\n\nНажмите на актив для получения сигнала",
            parse_mode="Markdown",
            reply_markup=create_commodities_menu(page)
        )
    except Exception:
        traceback.print_exc()
        bot.answer_callback_query(call.id, "❌ Произошла ошибка!")
...
@bot.callback_query_handler(func=lambda call: call.data.startswith("indices_page_"))
def handle_indices_pagination(call):
    try:
        page = int(call.data.split("_")[2])
        bot.edit_message_text(
            chat_id=call.message.chat.id,
            ...
            text="📊 **Выберите индекс:**\n\nНажмите на актив для получения сигнала",
            parse_mode="Markdown",
            reply_markup=create_indices_menu(page)
        )
    except Exception:
        traceback.print_exc()
        bot.answer_callback_query(call.id, "❌ Произошла ошибка!")
...
def handle_timeframe_callback(call):
    try:
        ...
        bot.send_message(
            call.message.chat.id,
            f"✅ Настройки сохранены!\n\n⏱️ Теперь вы будете получать сигналы с экспирацией: **{esc_md(timeframe_text)}**",
            parse_mode="Markdown",
            reply_markup=create_main_menu()
        )
    ...
    try:
        ...
        for req in pending_requests:
            ...
            response = (
                f"👤 **{esc_md(first_name)}**\n"
                f"├ ID: `{user_id}`\n"
                f"├ Username: @{esc_md(username if username else 'нет')}\n"
                f"├ Pocket ID: {esc_md(pocket_id)}\n"
                f"└ Запрос: {esc_md(request_date)}\n"
            )
